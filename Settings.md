...
```
This setting configures a webhook at the endpoint `https://cloudomation.com/api/1/webhook/<client name>/<webhook name>`. A `GET` or `POST` request to that URL will execute a flow script. The webhook will return the flow with all inputs and outputs as a json response. If no response is required, adding 'async' as a parameter to the call will only trigger the flow and return as response only the execution id.  
