...
You can see how many Token you are currently using by looking at the "Token usage counter" at the top left of the Cloudomation user interface while you are logged in. If you hover over the displayed percentage of your Token usage, you will see the count of Token currently in use as well as your total number of Token.  
...
You can check how many Token you have in your account in the [Token](/token) section while you are logged in. There, you see how many Token you have and how long they are valid for. Token are usually valid for one month. If you don't cancel your subscription, you Token will automatically be renewed at the end of each month for another month.
...
|execution\_task\_input|Exeution of an INPUT task.|1|Calling an INPUT task in your flow script will start a child execution for an INPUT task. This child execution will block 1 Token for as long as it runs. It will run until it receives a response, or until it times out. Depending on the responsiveness of the user, and the timeout you define, this can taker more or less time.|Short term: only while the INPUT task is running.|
|execution\_task\_aws|Execution of an AWS task.|2|Calling an AWS task in your flow script will start a child execution for the AWS task. This child execution will block 2 Token for as long as it runs. If the parent execution starts the AWS task synchronously, the parent execution will change into the status "waiting for dependency" until the AWS task completes. While the parent execution is in the "waiting for dependency" status it will not block any Token.|Short term: only while the AWS task is running.|
|execution\_task\_redis|Execution of a REDIS task.|2|Same as the AWS task.|
|execution\_task\_rest|Execution of a REST task.|1|Same as the AWS task - except: the child execution will block only 1 Token.|Short term: only while the REST task is running|
|execution\_task\_smtp|Execution of an SMTP task.|2|Same as the AWS task.|Short term: only while the SMTP task is running|
|execution\_task\_ssh|Execution of an SSH task.|2|Same as the AWS task.|Short term: only while the SSH task is running|
|execution\_task\_git|Execution of a GIT task.|3|Same as the AWS task - except: the child execution will block 3 Token.|Short term: only while the GIT task is running|
|execution\_task\_scp|Execution of an SCP task.|2|Same as the AWS task|Short term: only while the SCP task is running|
|execution\_task\_k8s|Execution of a KUBERNETES (k8s) task.|3|Same as the AWS task - except: the child execution will block 3 Token.|Short term: only while the KUBERNETES task is running|
|files_gb|Used storage space for files. You can see and organise your files in the [Files](/files) view.|1|1 GB of stored files will block 1 Token. File storage is measured in 1/100 steps up to the GB, meaning that the first file you store on the Cloudomation platform, 0.01 (1/100) Token will be blocked, until you store files using more than 1/100 GB of storage, from when on the next 0.01 Token will be blocked, and so on.|Intermediate to long term: until you actively delete files. Cloudomation is not intended as a file storage solution. We therefore recommend not keeping files stored on the platform for longer term. The file storage functionality is intended to enable you to use files in your automations - however we recommend removing them when they are no longer immediately used by a flow script.|
|socket|An active web sockets. Web sockets are used to enable live views in the user interface, such as e.g. the Executions view, which is updated live via a web socket to display changes in your executions immediately as they occur.|0.01|Every live view in the user interface opens one active web socket, costing 0.01 token. You can recognise live views by the green circle icon next to them in the left hand menu pane. You can free up Token by closing live views. Please note that being logged in to the Cloudomation user interface in a browser window even when you don't use it means that you block Token by accessing the live views generated for your browser window.|Short to intermediate term: only as long as you have a live view open in your browser window.|

Please note that the Token prices block the mentioned amount of Token only **while the respective method is running**. For example, if you use a git task in your flow script, and afterwards you use a REST task, running this flow script will first block 3 Token while the git task is active, free up those 3 Token as soon as the git task is done, and then block 1 Token for the REST task while the REST task is active. It will not block 4 Token right away, or at the same time (unless you specify them to run in parallel). You will need only 3 free Token in total to run such a flow script.
